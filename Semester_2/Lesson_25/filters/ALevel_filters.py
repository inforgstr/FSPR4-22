...
def save_json(result: list[dict], file_name="file", indent=4) -> None:
    """
    Save result list/dict to JSON file

    Args:
        result: Dictionaries of List
        file_name: File name to save json file by this name
        indent: amount of indents in json file
    ...
    try:
        ...
        with open(f"{base}/results/{file_name}.json", "w") as file:
...
def find_similars(data: list[dict]) -> str:
    ...
    # Call save_json function to save result into JSON
    save_json(result)
